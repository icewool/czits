...
uses
  ...
  MessageDigest_5, uQTZHelper, System.NetEncoding;
...
type
  ...
  public
    ...
    class procedure SaveVio1(list: Tlist<TPass>);
...
//    exit;
  result := false;
  if TQTZHelper.QTZUrl <> '' then
  begin
    ret := TQTZHelper.WriteVehicleInfo(
      device.BABH,
      device.BAFX,
      ...
      '');
    result := ret > 0;
    logger.Debug('[WriteVehicleInfo]:' + ret.ToString + ' ' + device.SBBH + ' ' + pass.hphm);
  end;
end;
...
var
  ret, WSResult: string;
  hole: HoleService;
  gateID, directID, driverWayId, carType: string;
begin
  Result := true;
  gateID := holeSBBH[value.kdbh];
  directID := copy(gateID, 19, 10);
  gateID := copy(gateID, 1, 18);
  driverWayId := value.kdbh.Substring(value.kdbh.Length-1);
  try
    hole := GetHoleService(false, holeUrl);
    WSResult := hole.InitTrans(gateID, directID, driverWayId, '');
    ...
    begin
      if value.hpzl = '01' then
        ...
        carType := '0';
      ret := hole.WriteVehicleInfo(gateID, directID,
        driverWayId,
        '00',
        value.hpzl,
...
class procedure Tmypint.SaveVio1(list: Tlist<TPass>);
  procedure exec(sql: TStrings);
  var
    s: string;
  begin
    s := sql.Text;
    s := copy(s, 1, length(s) - 3); // 回车换行
    s := 'INSERT INTO T_vio_temp_hik (WFSJ,CJJG,HPHM,HPZL,WFDD,WFXW,CD,XZSD,SJSD,CSYS,CLPP,ZGXS,ZDXS,FWQDZ,PHOTOFILE1,PHOTOFILE2,PHOTOFILE3,bj)VALUES' + s;
    SQLHelper.ExecuteSql(s);
    sql.Clear;
    logger.Info('SaveVio OK: ' + sql.Count.ToString);
  end;
...
begin
  ...
  begin
    sql.Add('('''
      ...
      + vio.WFXW + ''','''
      + vio.CDBH + ''','''
      + '0'','''      //+ inttostr(gDicDevice[vio.kdbh].XZSD) + ''','''
      + vio.clsd + ''','''
      + vio.csys + ''','''
      + vio.clpp + ''',''0'',''0'','''
      + vio.FWQDZ + ''','''
...
begin
  ...
  begin
    ...
    token := getToken;
    url := Format(smsUrl, [token, sj, TnetEncoding.URL.Encode(content)]);
    http := TIdHttp.Create(nil);
    stream := TMemoryStream.Create;
    try
