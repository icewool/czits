...
{$ENDIF}
  ...
  dxCustomTileControl, dxTileControl, Data.DB, uUserPower, UPGS,
  Vcl.ExtCtrls, uFrameMain, FireDAC.Comp.Client, uFrameQDZView,
...
begin
  FrameMain := TFrameMain.Create(pageMain.Pages[0]);
  FrameMain.Parent := pageMain.Pages[0];
  FrameMain.Align := TAlign.alClient;
  FrameMain.OnTitleClick := TitleItemClick;
...
var
  frame: TdxFrameInfo;
begin
  frame := dxFrameManager.GetFrameInfoByID(TdxNavBarItem(Sender).Tag);
  if frame <> nil then
  begin
