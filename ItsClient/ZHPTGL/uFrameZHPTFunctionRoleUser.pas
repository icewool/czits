unit uFrameZHPTFunctionRoleUser;
...
type
  ...
  TFrameZHPTFunctionRoleUser = class(TdxFrame)
...
{$R *.dfm}
{ TFrameZHPTFunctionRoleUser }

procedure TFrameZHPTFunctionRoleUser.AfterConstruction;
var
  xzqh, Param: String;
  depts: TList<TDept>;
  dept: TDept;
...
procedure TFrameZHPTFunctionRoleUser.btnSaveClick(Sender: TObject);
var
  YHBH: String;
begin
  inherited;
...
procedure TFrameZHPTFunctionRoleUser.btnSearchClick(Sender: TObject);
begin
  inherited;
  SearchUser;
end;

procedure TFrameZHPTFunctionRoleUser.cxButton1Click(Sender: TObject);
begin
  inherited;
  if not FDMemTable1.Active then
    exit;
...
procedure TFrameZHPTFunctionRoleUser.cxButton2Click(Sender: TObject);
begin
  inherited;
  if not FDMemTable1.Active then
    exit;
...
procedure TFrameZHPTFunctionRoleUser.GetRoleUsers;
var
...
begin
  ...
  users := TJsonUtils.JsonToRecordList<TRoleUser>
    (TRequestItf.DbQuery('GetZHPTFunctionRoleUser', 'RoleID=' + FRoleID));
  for user in users do
    FRoleUsers.Add(user.YHBH);
  users.Free;
end;

procedure TFrameZHPTFunctionRoleUser.InitTb;
begin
  FDMemTable1.Close;
...
procedure TFrameZHPTFunctionRoleUser.SearchUser(RoleID: String);
var
  Param: String;
begin
  inherited;
