...
begin
  ...
  s := IdHTTP1.Get
    (TIdURI.URLEncode
    ('http://127.0.0.1:20086/Login?user=250688&pwd=E14B236263F88BB55354B247CEE47C29')
    // ('http://127.0.0.1:20088/Login?user=sa&pwd=EB8E560662808ED4FAEA1C8AF016055947C4EF17CFD7AFC5390AA126AB25E2A2')
    );

  // s:= 'http://10.43.255.8:10088/Query?jkid=04C03&hpzl=02&hphm=粤U99858'
  // s:= 'http://10.43.255.8:10088/GetVehInfo?hpzl=02&hphm=粤U99858'
  s := 'http://127.0.0.1:20086/UploadVio?systemid=' + Trim(Edit1.Text) +
    '&token=' + s;
...
var
  ...
  AParams: TStrings;
  s: String;
  vioStr, code, sxh, zt, bz: String;
begin
  s := '{"root":{"head":{"msg":"该支队管理部门下不存在该道路代码！","code1":"0","level":"S",' +
    '"rank":"0","code":"0","msg1":"wfdd","displaymsg":"[0S1]:wfdd"}}}';
  // TUploadVio.DecodeZHPTResultHead(s, code, sxh);
